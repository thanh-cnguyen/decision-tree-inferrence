...
def split_partition(attributes, partitions):
    partition_id, attribute_id, dump = max_f_values(attributes, partitions)
    if dump is None:
        print('Invalid f_value')
    else:
        partition_list = partitions[partition_id]
        ...
        ids = []
        for _ in res.keys():
            id = rd.randrange(len(attributes[-1]))
            while id in ids or str(id) in partitions.keys():
                id = rd.randrange(len(attributes[-1]))
            ids.append(id)

        partitions.pop(partition_id, 'Not found partition to remove')
        for val in ids:
            partitions[val] = res.popitem()[1]
        if len(ids) == 2:
            print(
                f'Partition {partition_id} was replaced with partitions {ids[0]} and {ids[1]} using Attribute {attribute_id}.')
        elif len(ids) == 3:
            print(
                f'Partition {partition_id} was replaced with partitions {ids[0]}, {ids[1]}, and {ids[2]} using Attribute {attribute_id}.')
